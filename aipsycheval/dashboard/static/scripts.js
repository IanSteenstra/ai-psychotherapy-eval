...
document.addEventListener('DOMContentLoaded', function() {
    ...
    Object.assign(THERAPIST_DISPLAY_NAMES, window.THERAPIST_LABELS || {});
    ...
    const FALLBACK_THERAPIST_COLORS = [
        [54, 162, 235], [255, 99, 132], [75, 192, 192], [255, 159, 64], [153, 102, 255],
        [255, 205, 86], [46, 139, 87], [220, 20, 60], [0, 128, 128], [128, 0, 128]
    ];

    function getTherapistColors(therapistId) {
        if (THERAPIST_COLOR_MAP[therapistId]) {
            return THERAPIST_COLOR_MAP[therapistId];
        }
        if (!therapistId) {
            return { background: 'rgba(201, 203, 207, 0.7)', border: 'rgba(201, 203, 207, 1)' };
        }
        // Stable color per therapist id for runs with custom therapists.
        let hash = 0;
        for (const char of String(therapistId)) {
            hash = (hash * 31 + char.charCodeAt(0)) >>> 0;
        }
        const [r, g, b] = FALLBACK_THERAPIST_COLORS[hash % FALLBACK_THERAPIST_COLORS.length];
        return { background: `rgba(${r}, ${g}, ${b}, 0.7)`, border: `rgba(${r}, ${g}, ${b}, 1)` };
    }
